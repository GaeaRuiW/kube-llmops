...
- [ ] Repo scaffolding, Makefile, CI, lint
- [ ] **Model Resolver** init-container (format detection + engine auto-selection)
- [ ] vLLM Helm sub-chart + llama.cpp Helm sub-chart
- [ ] LiteLLM Helm sub-chart (Tier 1 gateway, PostgreSQL)
- [ ] **OpenTelemetry Collector** + Prometheus + DCGM Exporter
- [ ] Grafana + 3 dashboards (Overview, GPU Fleet, Per-Model)
- [ ] Umbrella Chart + `values-minimal.yaml`
- [ ] Quick start guide + README
- [ ] `scripts/install.sh`

### Phase 2: Production Readiness -- "Run in production"
- [ ] Multi-model deployment (vLLM + TEI + llama.cpp, auto-selected)
- [ ] LiteLLM advanced (load balancing, fallback, budget, rate limiting)
- [ ] **Langfuse** integration (LiteLLM callback + OTel OTLP)
- [ ] **Fluentbit** + Loki logging pipeline
- [ ] Full 6 Grafana dashboards + alert rules
- [ ] **KEDA** autoscaling (pending requests, TTFT)
- [ ] **Fluid** distributed model caching
- [ ] MinIO + **Harbor** (model registry)
- [ ] `values-standard.yaml`
- [ ] Keycloak SSO, Cilium NetworkPolicy

...
| 5.5 | Dashboard: LLM Cluster Overview | `dashboards/llm-cluster-overview.json` |
|     | - Total requests/sec, active models count, total tokens/sec | |
|     | - Cluster-wide GPU utilization, error rate, model status | |
| 5.6 | Dashboard: GPU Fleet | `dashboards/gpu-fleet.json` |
|     | - Per-node GPU util%, VRAM, temperature, power draw | |
|     | - XID error counter, MIG status | |
| 5.7 | Dashboard: Per-Model Deep Dive | `dashboards/per-model-deep-dive.json` |
